...
function loadBrowserTiles(filename) {
  ...
  const allFeatures = [];
  Object.values(window.browserGeneratedTiles).forEach(zoomLevel => {
    Object.values(zoomLevel).forEach(tile => {
      if (tile.layers && tile.layers.osm) {
        tile.layers.osm.features.forEach(feature => {
          const convertedFeature = convertTileFeatureToGeo(feature, tile.x, tile.y, tile.z);
          if (convertedFeature) {
            allFeatures.push(convertedFeature);
          }
        });
      }
    });
  ...
  document.getElementById('mapInfoContent').innerHTML = `
    ...
    <p><strong>Features:</strong> ${uniqueFeatures.length.toLocaleString()}</p>
    <p><strong>🔴</strong> Points • <strong>🟣</strong> Roads • <strong>🟡</strong> Areas</p>
  `;
...
function addStyledLayers(sourceId) {
  ...
  map.addLayer({
    ...
    paint: {
      'line-color': [
        'case',
        ['==', ['get', 'highway'], 'primary'], '#ff3838',
        ['==', ['get', 'highway'], 'secondary'], '#ff6b9d',
        ['==', ['get', 'highway'], 'residential'], '#a55eea',
        ['has', 'highway'], '#5f27cd',
        ['has', 'railway'], '#00d2d3',
        '#ddd'
      ],
...
function removeCurrentLayer() {
  if (currentTileLayer) {
    const layerIds = [`${currentTileLayer}-points`, `${currentTileLayer}-lines`, `${currentTileLayer}-polygons`];
...
// Toggle map style (simple implementation)
let currentMapStyle = 'osm';
function toggleMapStyle() {
  if (currentMapStyle === 'osm') {
    // Switch to satellite/terrain style
    map.setStyle({
      version: 8,
      sources: {
        'satellite': {
